...
function isLightColor(color = '#555555'){
  color = color.match(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i) || color.match(/^#?([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})$/i).map((i)=>{ return `${i}${i}`;});
  return !!Math.round((parseInt(color[1], 16) * 299 + parseInt(color[2], 16) * 587 + parseInt(color[3], 16) * 114) / 255000);
}
...
function makeBadge (measurer, {
  ...
  const context = {
    ...
    logo,
    logoInvert: !!isLightColor(colorA),
    logoWidth,
    logoPadding,
    colorA,
    colorB,
